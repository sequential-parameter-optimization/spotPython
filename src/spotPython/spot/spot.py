...
from spotPython.utils.transform import nat_to_cod_X, nat_to_cod_y, cod_to_nat_y
...
class Spot:
    ...
    * Displaying information. The `plot` method can be used for visualizing results. The `print` methods summarizes
    ...
    Args:
        ...
        cod_type (str):
            Normalize, standardize or do not modify variables passed to the surrogate:
            Can be "norm", "std", or "id". Default is "id" (no modification).
        ocba_delta (int):
    ...
    def __init__(
        ...
        self.range_X = []
        self.range_y = []
        # cod_to_nat variables:
        self.cod_min_X = None
        self.cod_max_X = None
        self.cod_min_y = None
        self.cod_max_y = None
        self.cod_mean_X = None
        self.cod_std_X = None
        self.cod_mean_y = None
        self.cod_std_y = None
        # Bounds
        de_bounds = []
    ...
    def fit_surrogate(self):
        ...
        cod_X, min_X, max_X, mean_X, std_X = nat_to_cod_X(X=X, cod_type=self.cod_type)
        cod_y, min_y, max_y, mean_y, std_y = nat_to_cod_y(y=y, cod_type=self.cod_type)
        self.cod_min_X = min_X
        self.cod_max_X = max_X
        self.cod_min_y = min_y
        self.cod_max_y = max_y
        self.cod_mean_X = mean_X
        self.cod_std_X = std_X
        self.cod_mean_y = mean_y
        self.cod_std_y = std_y
        self.surrogate.fit(cod_X, cod_y)
    ...
    def infill(self, x):
        ...
        if isinstance(self.surrogate, Kriging):
            # res = self.surrogate.predict(cod_X, return_val=self.infill_criterion)
            cod_X, min_X, max_X, mean_X, std_X = nat_to_cod_X(X=X, cod_type=self.cod_type)
            cod_y = self.surrogate.predict(cod_X, return_val=self.infill_criterion)
            res = cod_to_nat_y(
                cod_y=cod_y,
                cod_type=self.cod_type,
                min_y=self.cod_min_y,
                max_y=self.cod_max_y,
                mean_y=self.cod_mean_y,
                std_y=self.cod_std_y,
            )
        else:
